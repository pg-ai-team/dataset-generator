...
def genPhotos():
    ...
    kegg_json = os.path.join(raw_dir, "kegg_pathways.json")
    ...
    sample_groups = None
    sample_info_path = os.path.join(raw_dir, "sampleInfo.csv")

    if os.path.exists(sample_info_path):
        sample_info = pd.read_csv(sample_info_path, sep=";")
        if "smallCell" in sample_info.columns:
            print("jest plik")
            if len(sample_info) == data_filtered_annotated.shape[1]:
                ...
                if multiClass:
                    ...
                    sample_groups = sample_info["smallCell"].tolist()
            else:
                print("Warning: sample_info length mismatch – ignoring groups.")
    else:
        print("No sample_info2.csv found – files will be named by sample only.")
